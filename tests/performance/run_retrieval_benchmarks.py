...
from Medical_KG_rev.services.evaluation.runner import EvaluationConfig, EvaluationResult, EvaluationRunner
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--test-set', default='test_set_v1', help='Name of the evaluation dataset to load')
    parser.add_argument('--dataset-root', default=None, help='Filesystem path containing test set YAML files')
    parser.add_argument('--top-k', type=int, default=10, help='Number of documents to request per query')
    parser.add_argument('--rerank', action='store_true', help='Enable reranking during the benchmark run')
    parser.add_argument('--rerank-model', default=None, help='Optional reranker model override')
    parser.add_argument('--metrics-endpoint', default=None, help='Prometheus metrics endpoint (defaults to <base-url>/metrics)')
    parser.add_argument('--output', default=None, help='Optional JSON file to write benchmark results to')
    parser.add_argument('--timeout', type=float, default=30.0, help='HTTP timeout in seconds for retrieval requests')
    parser.add_argument('--bootstrap-samples', type=int, default=0, help='Bootstrap samples for EvaluationRunner (0 disables)')
    parser.add_argument('--markdown-output', default=None, help='Optional Markdown file for human-readable summary')
...
class ResponseCollector:
    ...
    def record(self, response: Mapping[str, Any]) -> None:
        ...
        component_timings_ms = components.get('timings_ms') if isinstance(components, Mapping) else {}
...
def scrape_prometheus(endpoint: str, headers: Mapping[str, str] | None, metric_names: Sequence[str]) -> dict[str, list[float]]:
...
def run_benchmark(args: argparse.Namespace) -> tuple[EvaluationResult, ResponseCollector, dict[str, list[float]]]:
    ...
    prom_metrics = scrape_prometheus(metrics_endpoint, headers if args.token else None, [
