    ...
    def __init__(
        ...
        *,
        ...
        if active_namespaces is not None:
            base_namespaces = list(active_namespaces)
        elif embedding_worker is not None:
            base_namespaces = list(getattr(embedding_worker, "active_namespaces", []) or [])
        else:
            base_namespaces = [vector_namespace]
        if vector_namespace not in base_namespaces:
            base_namespaces.append(vector_namespace)
        self.active_namespaces = base_namespaces
        if namespace_weights is not None:
            weights = dict(namespace_weights)
        elif embedding_worker is not None:
            weights = dict(getattr(embedding_worker, "namespace_weights", {}) or {})
        else:
            weights = {}
        for namespace in self.active_namespaces:
            weights.setdefault(namespace, 1.0)
        self.namespace_weights = weights
