...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--tenant-id', default='eval', help='Tenant identifier used for the evaluation request')
    parser.add_argument('--token', default=None, help='Bearer token for authenticated deployments')
    parser.add_argument('--test-set', default='test_set_v1', help='Name of the packaged evaluation dataset to use')
    parser.add_argument('--dataset-root', default=None, help='Optional filesystem root containing custom test set YAML files')
    parser.add_argument('--top-k', type=int, default=10, help='Number of documents to evaluate (Recall@K baseline)')
    ...
    parser.add_argument('--output', default=None, help='File path to persist the evaluation response as JSON')
