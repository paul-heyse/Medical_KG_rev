...
from Medical_KG_rev.models.overlays.legal import LegalClause, LegalDocument, LegalReference
...
def test_medical_document_limit_assessments():
    ...
    document = MedicalDocument(
        id="d1", source="clinicaltrials", sections=[], evidence_assessments=assessments
    )
...
def test_financial_document_context_validation():
    ...
    with pytest.raises(ValueError):
        FinancialDocument(
            ...
            sections=[],
            contexts=contexts,
            facts=[FinancialFact(concept="R", value="1", context_ref="missing")],
        )


def test_legal_document_clause_deduplication():
    clause = LegalClause(
        id="c1", references=[LegalReference(target="CaseA"), LegalReference(target="CaseA")]
    )
    document = LegalDocument(
        id="l1", source="lex", sections=[], clauses=[clause], jurisdiction="us-ca"
    )
