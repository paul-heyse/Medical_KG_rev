...
This script generates CA certificate and service certificates for
mutual TLS authentication between GPU services.
"""
...
async def generate_ca_certificate(
    ...
    ca_key_path: str,
    key_size: int = 2048,
    duration_days: int = 3650,  # 10 years for CA
) -> None:
...
async def main():
    ...
    parser.add_argument("--ca-cert", default="certs/ca.crt", help="Path to CA certificate file")
    parser.add_argument("--ca-key", default="certs/ca.key", help="Path to CA private key file")
    parser.add_argument(
        "--services",
        nargs="+",
    ...
    parser.add_argument(
        "--duration", type=int, default=365, help="Certificate validity duration in days"
    )
    parser.add_argument(
        "--ca-duration", type=int, default=3650, help="CA certificate validity duration in days"
    )
    parser.add_argument("--ca-only", action="store_true", help="Generate only CA certificate")
