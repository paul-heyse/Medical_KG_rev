...
def test_pdf_pipeline_declares_gate_configuration() -> None:
    ...
    assert {"download", "gate_vlm_ready"}.issubset(stage_names)

    assert topology.gates, "expected gate definitions for pdf-two-phase"
    gate = next(g for g in topology.gates if g.name == "vlm_processing_ready")
    assert gate.resume_stage == "chunk"
    assert gate.condition.field == "vlm_processing_ready"
