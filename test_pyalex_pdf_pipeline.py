...
class PyalexPipelineTester:
    ...
    def test_pyalex_import(self) -> bool:
        ...
        try:
            ...
            if pyalex_config.get("email") == self.email:
                ...
                logger.warning(
                    f"pyalex_email_not_set - expected: {self.email}, actual: {pyalex_config.get('email')}"
                )
                return False

        except ImportError as e:
            logger.error(f"pyalex_import_failed - error: {e!s}")
            return False
    ...
    def test_openalex_adapter_initialization(self) -> bool:
        ...
        try:
            ...
            logger.info("openalex_adapter_initialized", email=self.email, adapter_name=adapter.name)
            return True
    ...
    def test_keyword_search(self) -> bool:
        ...
        try:
            ...
            if documents:
                ...
                pdf_count = sum(1 for doc in documents if doc.metadata.get("pdf_urls"))
                logger.info("pdf_availability", total_docs=len(documents), docs_with_pdf=pdf_count)
                return True
            else:
    ...
    def test_openalex_id_search(self) -> bool:
        ...
        try:
            ...
            logger.info("testing_openalex_id_search", openalex_id=context.parameters["openalex_id"])
            documents = self.adapter.fetch_and_parse(context)

            if documents:
                doc = documents[0]
                logger.info("openalex_id_search_success", document_id=doc.id, title=doc.title)
                return True
            else:
        ...
        try:
            ...
            if documents:
                ...
                if not missing_fields:
                    ...
                    logger.warning("metadata_fields_missing", missing_fields=missing_fields)
                    return False
            else:
